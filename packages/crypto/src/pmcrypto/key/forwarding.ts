...
const doesKeyPacketSupportForwarding = (maybeForwardeeKey: Key | Subkey) => {
    ...
    return (
        maybeForwardeeKey.keyPacket instanceof SecretSubkeyPacket && // only ECDH can forward, and they are always subkeys
...
async function getEncryptionKeysForForwarding(
    forwarderKey: PrivateKey,
    date: Date,
) {
    const forwarderEncryptionKeys = (
        await Promise.all(
            forwarderKey
                .getKeyIDs()
                .map((maybeEncryptionKeyID) =>
                    forwarderKey
                        .getEncryptionKey(maybeEncryptionKeyID, date)
                        .catch(() => null),
                ),
        )
 ...
 */
export async function doesKeySupportForwarding(
    forwarderKey: PrivateKey,
 ...
 */
export const isForwardingKey = async (
    keyToCheck: PrivateKey,
    date: Date = serverTime(),
) => {
    ...
    const allValidKeys = allDecryptionKeys.every(
        (key) =>
            doesKeyPacketSupportForwarding(key) &&
            hasForwardingKeyFlag(key as Subkey),
    );
...
export async function generateForwardingMaterial(
    ...
    const { privateKey: forwardeeKeyToSetup } = await generateKey({
        ...
        subkeys: new Array<SubkeyOptions>(forwarderEncryptionKeys.length).fill({
            curve: curveName,
            forwarding: true,
        }),
        format: "object",
    ...
    const proxyInstances = await Promise.all(
        forwarderEncryptionKeys.map(async (forwarderSubkey, i) => {
            const forwarderSubkeyPacket =
                forwarderSubkey.keyPacket as SecretSubkeyPacket;
            const forwardeeSubkeyPacket = forwardeeKeyToSetup.subkeys[i]
                .keyPacket as SecretSubkeyPacket;
            const forwarderKeyFingerprint =
                forwarderSubkeyPacket.getFingerprintBytes()!;
