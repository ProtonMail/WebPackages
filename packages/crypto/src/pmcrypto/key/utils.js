...
import { DEFAULT_KEY_GENERATION_OFFSET } from "../constants.ts";
import { getSymmetricKeySize, getRandomBytes } from "../crypto/utils.ts";
import encryptMessage from "../message/encrypt.js";
...
export function generateSessionKey({
    ...
    return openpgp_generateSessionKey({
 ...
 * @param {import('../openpgp').Key} publicKey - key to check
...
export function getMatchingKey(signature, keys) {
    ...
    for (const signingKeyID of keyIDs) {
        ...
        const signingKey = keys.find(
            (key) => key.getKeys(signingKeyID).length > 0,
        );
