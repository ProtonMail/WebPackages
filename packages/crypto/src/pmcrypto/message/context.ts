 ...
 */
export const getNotationForContext = (
    contextValue: string,
    critical: boolean,
): RawNotation => ({
 ...
 */
export const isValidSignatureContext = (
    contextOptions: ContextVerificationOptions,
    signature: SignaturePacket,
) => {
    const { value: expectedValue, required, requiredAfter } = contextOptions;
    const isContextRequired = requiredAfter
        ? signature.created! >= normalizeDate(requiredAfter)
        : required;
