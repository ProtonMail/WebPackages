...
import { VERIFICATION_STATUS, MAX_ENC_HEADER_LENGTH } from "../constants.ts";
import { serverTime } from "../serverTime.ts";
import { parseMail, generateFileName } from "./parseMail.ts";
import {
    readSignature,
    type PublicKey,
...
type VerifySignatureOptions = Pick<
    ProcessMIMEOptions,
    "verificationKeys" | "date"
>;
const verifySignature = async (
    { verificationKeys = [], date = serverTime() }: VerifySignatureOptions,
    data: string,
) => {
    const { headers } = await parseMail(
        data.split(/\r?\n\s*\r?\n/g)[0] + "\n\n",
    );
    ...
    const {
        ...
        (sigAttachment as
            | Attachment
            | undefined) /** explicit typing needed due to TS inference limitation with array destructuring */ ??
        {};
    if (
        sigAttachmentContentType.toLowerCase() !== "application/pgp-signature"
    ) {
        return {
            subdata: data,
            verificationStatus: VERIFICATION_STATUS.NOT_SIGNED,
...
const parse = async (
    ...
    for (const parsedAttachment of parsedAttachments) {
        ...
        const attachment = {
            ...
            contentId:
                parsedAttachment.contentId ||
                `<${crypto.getRandomValues(new Uint8Array(16)).toHex()}@pmcrypto>`,
        };
        ...
        const { from, subject: attachmentSubject } = await parseMail(
 ...
 * @param options.sender - the address of the sender of this message
 */
export default async function processMIME({
    data,
    ...options
}: ProcessMIMEOptions): Promise<ProcessMIMEResult> {
