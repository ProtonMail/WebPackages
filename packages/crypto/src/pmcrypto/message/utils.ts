...
export async function splitMessage(message: OpenPGPMessage) {
    ...
    const asymmetric = await splitPackets(
        message.packets.filterByTag(enums.packet.publicKeyEncryptedSessionKey),
    );
    const signature = await splitPackets(
        message.packets.filterByTag(enums.packet.signature),
    );
    const symmetric = await splitPackets(
        message.packets.filterByTag(enums.packet.symEncryptedSessionKey),
    );
    const compressed = await splitPackets(
        message.packets.filterByTag(enums.packet.compressedData),
    );
    const literal = await splitPackets(
        message.packets.filterByTag(enums.packet.literalData),
    );
