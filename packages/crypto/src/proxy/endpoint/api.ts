...
const toArray = <T>(maybeArray: MaybeArray<T>) =>
    Array.isArray(maybeArray) ? maybeArray : [maybeArray];

const getPublicKeyReference = async (
    key: PublicKey,
    keyStoreID: number,
    ...
    const keyContentHash = await SHA256(publicKey.write()).then((bytes) =>
        bytes.toHex(),
    );
...
const getPrivateKeyReference = async <
    V extends PrivateKeyReference = PrivateKeyReference,
>(
...
class KeyStore {
    ...
    clearAll() {
        this.store.forEach((key) => {
            if (key.isPrivate()) {
                // @ts-expect-error missing definition for clearPrivateParams()
...
type SerialisedOutputTypeFromFormat<F extends SerialisedOutputFormat> =
    F extends "armored"
        ? string
        : F extends "binary"
...
class KeyManagementApi {
    ...
    >(options: WorkerGenerateKeyOptions<CustomConfig>) {
        ...
        type KeyVersion = CustomConfig extends { v6Keys: true }
            ? PrivateKeyReferenceV6
            : PrivateKeyReferenceV4;
    ...
    async importPrivateKey<T extends Data>(
        ...
        if (expectDecrypted) {
            ...
            // @ts-expect-error missing .validate() types
    ...
    async importPublicKey<T extends Data>(
        {
            ...
            binaryKey,
            checkCompatibility = KeyCompatibilityLevel.NONE,
        }: WorkerImportPublicKeyOptions<T>,
        _customIdx?: number,
    ) {
    ...
    async exportPublicKey<F extends SerialisedOutputFormat = "armored">({
        ...
        const publicKey = maybePrivateKey.isPrivate()
            ? maybePrivateKey.toPublic()
            : maybePrivateKey;
        const serializedKey =
            format === "binary" ? publicKey.write() : publicKey.armor();
    ...
    async exportPrivateKey<F extends SerialisedOutputFormat = "armored">({
        ...
        const maybeEncryptedKey = doNotEncrypt
            ? privateKey
            : await encryptKey({ privateKey, passphrase });

        const serializedKey =
            format === "binary"
                ? maybeEncryptedKey.write()
                : maybeEncryptedKey.armor();
        return serializedKey as SerialisedOutputTypeFromFormat<F>;
    }
}
...
export class Api extends KeyManagementApi {
    ...
    >({
        ...
        config = {},
        ...options
    }: WorkerEncryptOptions<DataType> & {
        format?: FormatType;
        detached?: DetachedType;
    }) {
...
* Create a signature over the given data using `signingKeys`.
    ...
    >({
        signingKeys: signingKeyRefs = [],
        ...options
    }: WorkerSignOptions<DataType> & { format?: FormatType }) {
        const signingKeys = toArray(signingKeyRefs).map(
            (keyReference) =>
                this.keyStore.get(keyReference._idx) as PrivateKey,
        );
    ...
    >({
        ...
        } = await verifyMessage<DataType, FormatType>({
            signature,
            verificationKeys,
            ...options,
        });
        ...
        const serialisedResult = {
            ...
            signatures: signatureObjects.map(
                (sig) => sig.write() as Uint8Array<ArrayBuffer>,
    ...
    async decryptMessage<
        FormatType extends WorkerDecryptionOptions["format"] = "utf8",
    >({
        ...
        const signature =
            binarySignature || armoredSignature
                ? await getSignature({ binarySignature, armoredSignature })
                : undefined;
        ...
        } = await decryptMessage<Data, FormatType>({
            ...options,
            message,
            signature,
            encryptedSignature,
    ...
    async generateE2EEForwardingMaterial({
        ...
        const { proxyInstances, forwardeeKey } =
            await generateForwardingMaterial(
                originalKey,
                userIDsForForwardeeKey,
                date,
            );
     ...
     */
    async generateSessionKeyForAlgorithm(
        algoName: Parameters<typeof generateSessionKeyForAlgorithm>[0],
    ) {
    ...
    async generateSessionKey({
        ...
        const recipientKeys = toArray(recipientKeyRefs).map((keyReference) =>
    ...
    async encryptSessionKey<
        FormatType extends WorkerEncryptSessionKeyOptions["format"] = "armored",
    >({
        encryptionKeys: encryptionKeyRefs = [],
        ...options
    }: WorkerEncryptSessionKeyOptions & { format?: FormatType }): Promise<
    ...
    async processMIME({
        ...
        const { signatures: signatureObjects, ...resultWithoutSignature } =
            await processMIME({
                ...options,
                verificationKeys,
            });
    ...
    async getMessageInfo<DataType extends Data>({
        ...
        const message = await getMessage({ binaryMessage, armoredMessage });
        const signingKeyIDs = message
            .getSigningKeyIDs()
            .map((keyID) => keyID.toHex());
        const encryptionKeyIDs = message
            .getEncryptionKeyIDs()
            .map((keyID) => keyID.toHex());
    ...
    async getArmoredKeys({
        ...
        binaryKeys: Uint8Array<ArrayBuffer>;
    }) {
    ...
    async replaceUserIDs({
        ...
        targetKey.users = sourceKey.users.map((sourceUser) => {
            ...
            // @ts-expect-error as `mainKey` is supposed to be read-only
    ...
    async cloneKeyAndChangeUserIDs({
        ...
        // @ts-expect-error missing clone declaration
