...
interface CryptoProxyInterface extends CryptoApiInterface {
     ...
     */
    setEndpoint<T extends CryptoApiInterface>(
        endpoint: T,
        onRelease?: (endpoint: T) => Promise<void>,
    ): void;
...
>(options: WorkerVerifyOptions<DataType> & { format?: FormatType }) {
    const verificationResult = await assertNotNull(endpoint).verifyMessage<
        DataType,
        FormatType
    >(options);
    ...
    if (
        ...
        const fallbackverificationResult = await assertNotNull(
            endpoint,
        ).verifyMessage<string, FormatType>({
            ...options,
        ...
        if (
            fallbackverificationResult.verificationStatus ===
...
export const CryptoProxy: CryptoProxyInterface = {
    ...
    processMIME: async ({ date = serverTime(), ...opts }) =>
        assertNotNull(endpoint).processMIME({ ...opts, date }),
    ...
    generateSessionKeyForAlgorithm: async (opts) =>
        assertNotNull(endpoint).generateSessionKeyForAlgorithm(opts),
    ...
    importPrivateKey: async (opts) =>
        assertNotNull(endpoint).importPrivateKey(opts),
    importPublicKey: async (opts) =>
        assertNotNull(endpoint).importPublicKey(opts),
    generateKey: async ({
    ...
    exportPublicKey: async (opts) =>
        assertNotNull(endpoint).exportPublicKey(opts),
    exportPrivateKey: async (opts) =>
        assertNotNull(endpoint).exportPrivateKey(opts),
    ...
    replaceUserIDs: async (opts) =>
        assertNotNull(endpoint).replaceUserIDs(opts),
    cloneKeyAndChangeUserIDs: async (opts) =>
        assertNotNull(endpoint).cloneKeyAndChangeUserIDs(opts),
    generateE2EEForwardingMaterial: async ({ date = serverTime(), ...opts }) =>
        assertNotNull(endpoint).generateE2EEForwardingMaterial({
            ...opts,
            date,
        }),
    ...
    isRevokedKey: async ({ date = serverTime(), ...opts }) =>
        assertNotNull(endpoint).isRevokedKey({ ...opts, date }),
    isExpiredKey: async ({ date = serverTime(), ...opts }) =>
        assertNotNull(endpoint).isExpiredKey({ ...opts, date }),
    ...
    getArmoredMessage: async (opts) =>
        assertNotNull(endpoint).getArmoredMessage(opts),
    getArmoredKeys: async (opts) =>
        assertNotNull(endpoint).getArmoredKeys(opts),
    getArmoredSignature: async (opts) =>
        assertNotNull(endpoint).getArmoredSignature(opts),
    getSignatureInfo: async (opts) =>
        assertNotNull(endpoint).getSignatureInfo(opts),
    getMessageInfo: async (opts) =>
        assertNotNull(endpoint).getMessageInfo(opts),
