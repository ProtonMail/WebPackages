...
import type { AuthCredentials, AuthInfo } from "./interface.ts";
import { expandHash, hashPassword } from "./passwords.ts";
import { verifyAndGetModulus } from "./utils/modulus.ts";
import { checkUsername } from "./utils/username.ts";

const srpHasher = (arr: Uint8Array<ArrayBuffer>) => expandHash(arr);
...
export const getSrp = async (
    ...
    const hashedPasswordArray = await hashPassword({
        version: authVersion,
        password,
        salt: authVersion < 3 ? undefined : uint8ArrayToBinaryString(Uint8Array.fromBase64(Salt)),
