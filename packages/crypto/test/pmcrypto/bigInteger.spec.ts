...
import {
    ...
    modExp,
    modInv,
} from "../../src/pmcrypto/bigInteger.ts";
import { getRandomBytes } from "../../src/pmcrypto/crypto/utils.ts";
...
describe("BigInt utils", () => {
    ...
    it("toUint8Array is correct", () => {
        const nString =
            "";
    ...
    it("modExp is correct (large values)", () => {
        const stringX =
            "";
        const stringE =
            "";
        const stringN =
            "";
        const x = BigInt(stringX);
        const e = BigInt(stringE);
        const n = BigInt(stringN);

        const got = modExp(x, e, n);
        const expected = new BN(stringX)
            .toRed(BN.red(new BN(stringN)))
            .redPow(new BN(stringE));
