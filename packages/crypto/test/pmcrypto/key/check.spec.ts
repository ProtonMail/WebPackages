import { describe, it, expect } from "vitest";
import {
    checkKeyStrength,
    checkKeyCompatibility,
    readKey,
} from "../../../src/pmcrypto/index.ts";

// Test vector from https://www.ietf.org/archive/id/draft-ietf-openpgp-pqc-09.html#appendix-A.4.1
const pqcKeyV6 = `-----BEGIN PGP PRIVATE KEY BLOCK-----


V2JopLrNFBhMf52jpgAAAAAAAAAAAAAAAAAABA8TGSIp
-----END PGP PRIVATE KEY BLOCK-----`;

export const curve25519KeyV6 = `-----BEGIN PGP PUBLIC KEY BLOCK-----


-----END PGP PUBLIC KEY BLOCK-----`;

export const curve25519LegacyKeyV4 = `-----BEGIN PGP PUBLIC KEY BLOCK-----


/Rs9S+8arbQVoniq7izz3uisWxfjMup+IVEC5uqMld8L
=8+ep
-----END PGP PUBLIC KEY BLOCK-----`;

export const eddsaLegacyElGamalSubkeyV4 = `-----BEGIN PGP PRIVATE KEY BLOCK-----


=qRad
-----END PGP PRIVATE KEY BLOCK-----`;

export const rsa512BitsKeyV4 = `-----BEGIN PGP PUBLIC KEY BLOCK-----


=J9mf
-----END PGP PUBLIC KEY BLOCK-----`;

describe("key checks", () => {
    it("checkKeyStrength - it warns on insecure primary key (RSA 512 bits)", async () => {
        const key = await readKey({ armoredKey: rsa512BitsKeyV4 });
        expect(() => checkKeyStrength(key)).to.throw(
            /Keys shorter than 2047 bits are considered unsafe/,
        );
    });

    it("checkKeyStrength - it warns on insecure subkey (ElGamal)", async () => {
        const key = await readKey({ armoredKey: eddsaLegacyElGamalSubkeyV4 });
        expect(() => checkKeyStrength(key)).to.throw(
            /elgamal keys are considered unsafe/,
        );
    });

    it("checkKeyStrength - it does not warn on secure key (curve25519Legacy)", async () => {
        const v4Key = await readKey({ armoredKey: curve25519LegacyKeyV4 });
        expect(() => checkKeyStrength(v4Key)).to.not.throw();

        const v6Key = await readKey({ armoredKey: curve25519KeyV6 });
        expect(() => checkKeyStrength(v6Key)).to.not.throw();
    });

    it("checkKeyStrength - it does not warn on secure key (PQC)", async () => {
        const pqcKey = await readKey({ armoredKey: pqcKeyV6 });
        expect(() => checkKeyStrength(pqcKey)).to.not.throw();
    });

    it("checkKeyCompatibility - it rejects a v4 key using the new EdDSA format", async () => {
        const key = await readKey({
            armoredKey: `-----BEGIN PGP PUBLIC KEY BLOCK-----


=JIkN
-----END PGP PUBLIC KEY BLOCK-----`,
        });
        expect(() => checkKeyCompatibility(key)).to.throw(
            /key algorithm ed25519 is currently not supported/,
        );
        expect(() => checkKeyCompatibility(key, true)).to.throw(
            /key algorithm ed25519 is currently not supported/,
        );
    });

    it("checkKeyCompatibility - it rejects a v4 key with PQC encryption subkey", async () => {
        const key = await readKey({
            armoredKey: `-----BEGIN PGP PRIVATE KEY BLOCK-----


=Q0in
-----END PGP PRIVATE KEY BLOCK-----
`,
        });
        expect(() => checkKeyCompatibility(key)).to.throw(
            /key algorithm pqc_mlkem_x25519 is currently not supported/,
        );
        expect(() => checkKeyCompatibility(key, true)).to.throw(
            /key algorithm pqc_mlkem_x25519 is currently not supported/,
        );
    });

    it("checkKeyCompatibility - it rejects a v5 key", async () => {
        const key = await readKey({
            armoredKey: `-----BEGIN PGP PRIVATE KEY BLOCK-----


=wpkQ
-----END PGP PRIVATE KEY BLOCK-----`,
        });
        expect(() => checkKeyCompatibility(key)).to.throw(
            /Version 5 keys are currently not supported/,
        );
    });

    it("checkKeyCompatibility - it rejects a v6 key unless explicitly allowed", async () => {
        const key = await readKey({ armoredKey: curve25519KeyV6 });
        expect(() => checkKeyCompatibility(key)).to.throw(
            /Version 6 keys are currently not supported/,
        );
        expect(() => checkKeyCompatibility(key, true)).to.not.throw();
    });

    it("checkKeyCompatibility - it rejects a v6 PQC key unless explicitly allowed", async () => {
        const key = await readKey({ armoredKey: pqcKeyV6 });
        expect(() => checkKeyCompatibility(key)).to.throw(
            /Version 6 keys are currently not supported/,
        );
        expect(() => checkKeyCompatibility(key, true)).to.not.throw();
    });

    it("checkKeyCompatibility - it does not reject a v4 key using the eddsa legacy format", async () => {
        const key = await readKey({ armoredKey: curve25519LegacyKeyV4 });
        expect(() => checkKeyCompatibility(key)).to.not.throw();
    });
});
