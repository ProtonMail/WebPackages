...
import {
    decryptKey,
    enums,
    type PublicKeyEncryptedSessionKeyPacket,
    type KeyID,
    type PacketList,
...
import {
    computeProxyParameter,
    isForwardingKey,
} from "../../../src/pmcrypto/key/forwarding.ts";
import { mergeUint8Arrays } from "../../../src/utils.ts";
...
async function testProxyTransform(
    ...
    for (const packet of ciphertext.packets.filterByTag(
        ...
        if (packet.publicKeyID.equals(originalSubkeyID)) {
            ...
            const encoded = mergeUint8Arrays([new Uint8Array([0x40]), bkG]);
            // @ts-expect-error missing `encrypted` field
...
=pVRa
-----END PGP MESSAGE-----`;
        const charlieKey = await readPrivateKey({
            armoredKey: charlieKeyArmored,
        });
    ...
    it("proxy parameter computation is correct", () => {
        const secretBob = Uint8Array.fromHex(
            "5989216365053dcf9e35a04b2a1fc19b83328426be6bb7d0a2ae78105e2e3188",
        );
        const secretCharlie = Uint8Array.fromHex(
            "684da6225bcd44d880168fc5bec7d2f746217f014c8019005f144cc148f16a00",
        );

        const expectedProxyFactor = Uint8Array.fromHex(
            "e89786987c3a3ec761a679bc372cd11a425eda72bd5265d78ad0f5f32ee64f02",
        );
        const actualProxyFactor = computeProxyParameter(
            secretBob,
            secretCharlie,
        );
    ...
    it("generate forwarding key", async () => {
        ...
        const { forwardeeKey } = await generateForwardingMaterial(bobKey, [
            { name: "Charlie", email: "info@charlie.com" },
        ]);
        ...
        expect(charlieSubkey.bindingSignatures[0].keyFlags?.[0]).to.equal(
        ...
        expect(charlieSubkeyKDFParams.cipher).to.equal(
            bobSubkeyKDFParams.cipher,
        );
        expect(charlieSubkeyKDFParams.version).to.equal(0xff);
        expect(charlieSubkeyKDFParams.replacementFingerprint).to.deep.equal(
            bobSubkey.keyPacket.getFingerprintBytes(),
        );
    });
    ...
    it("generate forwarding key - KDF params hash and cipher are correctly copied over", async () => {
        ...
        const bobKey = await readPrivateKey({
...
-----END PGP PRIVATE KEY BLOCK-----`,
    ...
    it("generate forwarding key - should throw for P256 encryption key", async () => {
        const keyWithP256Subkey = await readPrivateKey({
            armoredKey: `-----BEGIN PGP PRIVATE KEY BLOCK-----
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        await expect(
            generateForwardingMaterial(keyWithP256Subkey, [
                { name: "Charlie", email: "info@charlie.com" },
            ]),
    ...
    it("decryption with forwarding - v4 key", async () => {
        ...
        const { proxyInstances, forwardeeKey: charlieKey } =
            await generateForwardingMaterial(bobKey, [
                {
                    name: "Charlie",
                    email: "info@charlie.com",
                    comment: "Forwarded from Bob",
                },
            ]);
        ...
        expect(proxyInstances[0].forwarderKeyFingerprint.toHex()).to.include(
            bobKey.subkeys[0].getKeyID().toHex(),
        );
        expect(proxyInstances[0].forwardeeKeyFingerprint.toHex()).to.include(
            charlieKey.subkeys[0].getKeyID().toHex(),
        );
        ...
        const { data: decryptedData } = await decryptMessage({
            message: await readMessage({
                armoredMessage: transformedCiphertext,
            }),
            decryptionKeys: charlieKey,
            config: { allowForwardedMessages: true },
        });
        ...
        await expect(decryptionTrialPromise).rejects.toThrow(
    ...
    it("decryption with forwarding - v4 key with multiple subkeys", async () => {
        ...
        proxyInstances.forEach((proxyInstance, i) => {
            ...
            expect(proxyInstance.forwarderKeyFingerprint.toHex()).to.include(
                bobForwardedSubkeys[i].getKeyID().toHex(),
            );
            expect(proxyInstance.forwardeeKeyFingerprint.toHex()).to.include(
                charlieKey.subkeys[i].getKeyID().toHex(),
            );
        });
        ...
        const { data: decryptedData1 } = await decryptMessage({
            message: await readMessage({
                armoredMessage: transformedCiphertext1,
            }),
            decryptionKeys: charlieKey,
            config: { allowForwardedMessages: true },
        });
        ...
        // @ts-expect-error missing `clone` definition
        ...
        const { data: decryptedData2 } = await decryptMessage({
            message: await readMessage({
                armoredMessage: transformedCiphertext2,
            }),
            decryptionKeys: charlieKey,
            config: { allowForwardedMessages: true },
        });
    ...
    it("supports forwarding - should return false for key without encryption subkeys", async () => {
        ...
        const keyWithoutEncryptionSubkey = await readPrivateKey({
            armoredKey: `-----BEGIN PGP PRIVATE KEY BLOCK-----
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        expect(await doesKeySupportForwarding(keyWithoutEncryptionSubkey)).to.be
            .false;
    });
    ...
    it("supports forwarding - should return false for encryption subkey without private key material (gnu-dummy)", async () => {
        ...
        const keyWithDummySubkey = await readPrivateKey({
            armoredKey: `-----BEGIN PGP PRIVATE KEY BLOCK-----
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        const charlieKeyEncrypted = await readPrivateKey({
            armoredKey: `-----BEGIN PGP PRIVATE KEY BLOCK-----
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        const charlieKey = await decryptKey({
            privateKey: charlieKeyEncrypted,
