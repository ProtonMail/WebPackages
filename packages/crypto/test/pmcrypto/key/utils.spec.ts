...
-----END PGP PRIVATE KEY BLOCK-----`,
    ...
    it("generateKey - it has valid default creation time", async () => {
        ...
        expect(Math.abs(+privateKey.getCreationTime() - +now) < 24 * 3600).to.be
            .true;
    });
    ...
    it("generateKey - v4 key - it includes the BE-expected algorithm preferences", async () => {
        ...
        const { selfCertification } =
            await privateKey.getPrimaryUser(serverTime());
        expect(selfCertification.preferredSymmetricAlgorithms).to.include(
            enums.symmetric.aes256,
        );
        expect(selfCertification.preferredHashAlgorithms).to.include(
            enums.hash.sha256,
        );
        expect(selfCertification.preferredCompressionAlgorithms).to.include(
            enums.compression.zlib,
        );
        // for v4 keys, we guard against sha3 being in the preferences, as it's not supported by all Proton clients
        expect(selfCertification.preferredHashAlgorithms).to.not.include(
            enums.hash.sha3_256,
        );
        expect(selfCertification.preferredHashAlgorithms).to.not.include(
            enums.hash.sha3_512,
    ...
    it("generateKey - v6 key - it includes the BE-expected algorithm preferences", async () => {
        ...
        // @ts-expect-error missing `directSignatures` type definition
        const [directSignature] = await privateKey.directSignatures;
        expect(directSignature.preferredSymmetricAlgorithms).to.include(
            enums.symmetric.aes256,
        );
        expect(directSignature.preferredHashAlgorithms).to.include(
            enums.hash.sha256,
        );
        expect(directSignature.preferredCompressionAlgorithms).to.include(
            enums.compression.zlib,
        );
        expect(directSignature.preferredHashAlgorithms).to.include(
            enums.hash.sha3_256,
    ...
    it("reformatKey - it reformats a key using the key creation time", async () => {
        ...
        const { privateKey: reformattedKey } = await reformatKey({
            privateKey,
            passphrase: "123",
        ...
        expect(
            (await reformattedKey.getPrimaryUser()).selfCertification.created,
        ).to.deep.equal(date);
    });
    ...
    it("isExpiredKey - it can correctly detect an expired key", async () => {
        ...
        expect(await isExpiredKey(expiringKey, new Date(+now + 1000))).to.be
            .true;
        expect(await isExpiredKey(expiringKey, new Date(+now - 1000))).to.be
            .true;
...
-----END PGP PRIVATE KEY BLOCK-----`;
        ...
        expect(
            signatureFromSubkey
                .getSigningKeyIDs()
                .includes(key1.subkeys[0].getKeyID()),
        ).to.be.true;
        expect(getMatchingKey(signatureFromSubkey, [key1, key2])).to.deep.equal(
            key1,
        );
        expect(
            signatureFromPrimaryKey
                .getSigningKeyIDs()
                .includes(key2.getKeyID()),
        ).to.be.true;
        expect(
            getMatchingKey(signatureFromPrimaryKey, [key1, key2]),
        ).to.deep.equal(key2);
    });
