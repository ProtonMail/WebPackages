...
describe("context", () => {
    it("signMessage/verifyMessage - it verifies a message with context (critical)", async () => {
        ...
        expect(verificationWrongContext.verificationStatus).to.equal(
        ...
        expect(verificationWrongContext.errors?.[0]).to.be.instanceOf(
            SignatureContextError,
        );
    ...
    it("signMessage/verifyMessage - it verifies a message with context (non critical)", async () => {
        ...
        expect(verificationWrongContextNotRequired.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_INVALID,
        );
        expect(verificationMissingContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        ...
        expect(
            verificationWrongContextNotRequired.errors?.[0],
        ).to.be.instanceOf(SignatureContextError);
    ...
    it("encryptMessage/decryptMessage - it verifies a message with context", async () => {
        ...
        expect(decryptionValidContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        );
        expect(decryptionWrongContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_INVALID,
        );
        expect(decryptionMissingContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_INVALID,
        );
        // check errors
        ...
        expect(decryptionWrongContext.verificationErrors?.[0]).to.be.instanceOf(
            SignatureContextError,
        );
        expect(decryptionMissingContext.verificationErrors).to.have.length(1);
        expect(decryptionMissingContext.verificationErrors?.[0]).to.match(
            /Unknown critical notation: context@proton/,
        );
    });
    ...
    it("encryptMessage/decryptMessage - it verifies an encrypted signature with context", async () => {
        ...
        const { message: armoredMessage, encryptedSignature } =
        ...
        const decryptionValidContext = await decryptMessage({
            message: await readMessage({ armoredMessage }),
            encryptedSignature: await readMessage({
                armoredMessage: encryptedSignature,
            }),
            decryptionKeys: privateKey,
            verificationKeys: publicKey,
            signatureContext: { value: "test-context", required: true },
...
-----END PGP MESSAGE-----`,
    ...
    it("does not verify a message without context", async () => {
        ...
        expect(verificationExpectedContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_INVALID,
        );
        expect(verificationNoExpectedContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        );

        expect(verificationNoExpectedContext.errors).to.be.undefined;
        expect(verificationExpectedContext.errors).to.have.length(1);
        expect(verificationExpectedContext.errors?.[0]).to.be.instanceOf(
            SignatureContextError,
        );
    });
...
-----END PGP PUBLIC KEY BLOCK-----`,
        ...
        expect(verificationWithContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_INVALID,
        );
        expect(verificationWithoutContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        );
    });
});
