...
describe("message encryption and decryption", () => {
    ...
    it("it can encrypt and decrypt a binary message", async () => {
        ...
        expect(decrypted).to.deep.equal(utf8StringToUint8Array("Hello world!"));
        expect(verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        );
    });
    ...
    it("it does not encrypt with SEIPDv2 by default", async () => {
        const privateKey = await readPrivateKey({
            armoredKey: v4PrivateKeySEIPDv2,
        });
    ...
    it("can encrypt with argon2 s2k", async () => {
        ...
        const encrypted = await encryptSessionKey({
            ...sessionKey,
            passwords,
            config,
            format: "object",
        ...
        // @ts-expect-error missing s2k field declaration
    ...
    it("it can encrypt and decrypt a message with a given session key", async () => {
        ...
        const sessionKey = {
            data: Uint8Array.fromHex(
                "c5629d840fd64ef55aea474f87dcdeef76bbc798a340ef67045315eb7924a36f",
            ),
    ...
    it("it can encrypt a message with a session key from `generateSessionKey`", async () => {
        ...
        const sessionKey = await generateSessionKey({
            recipientKeys: decryptedPrivateKey,
        });
    ...
    it("it can encrypt and decrypt a message with an unencrypted detached signature", async () => {
        ...
        const { message: encrypted, signature: armoredSignature } =
        ...
        const { verificationStatus: verificationStatusAgain } =
            await verifyMessage({
                textData: "Hello world!",
                signature: await readSignature({ armoredSignature }),
                verificationKeys: [decryptedPrivateKey.toPublic()],
            });
        expect(verificationStatusAgain).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        );
    });
    ...
    it("it can encrypt and decrypt a message with an encrypted detached signature", async () => {
        ...
        const parsedEncryptedSignature = await readMessage({
    ...
    it("it can encrypt and decrypt a streamed message with an unencrypted detached signature (format = armored)", async () => {
        ...
        const { data: decrypted, verificationStatus } = await decryptMessage({
            message: await readMessage({
                armoredMessage: await readToEnd(encrypted),
            }),
            encryptedSignature: await readMessage({
                armoredMessage: encryptedSignature,
            }),
        ...
        expect(uint8ArrayToUtf8String(await readToEnd(decrypted))).to.equal(
            inputData,
        );
        expect(verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
    ...
    it("it can encrypt and decrypt a streamed message with an unencrypted detached signature (format = binary)", async () => {
        ...
        const { data: decrypted, verificationStatus } = await decryptMessage({
            message: await readMessage({
                binaryMessage: await readToEnd(encrypted),
            }),
            encryptedSignature: await readMessage({
                binaryMessage: encryptedSignature,
            }),
            sessionKeys: sessionKey,
            verificationKeys: [decryptedPrivateKey.toPublic()],
            format: "binary",
    ...
    it("it can encrypt and decrypt a binary streamed message with an encrypted detached signature", async () => {
        ...
        const { data: decrypted, verificationStatus } = await decryptMessage({
            message: await readMessage({
                armoredMessage: toStream(
                    encryptedArmoredMessage,
                ) as WebStream<string>,
            }),
            encryptedSignature: await readMessage({
                armoredMessage: encryptedSignature,
        ...
        expect(await verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        );
    });
