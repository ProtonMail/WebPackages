...
import { readKey, processMIME } from "../../../src/pmcrypto/index.ts";
import { VERIFICATION_STATUS } from "../../../src/pmcrypto/constants.ts";
import { Signature } from "../../../src/pmcrypto/openpgp.ts";
import {
...
import { uint8ArrayToUtf8String } from "../../../src/utils.ts";
...
--------------cJMvmFk1NneB7MT4jwYHY7ap--`,
    ...
    it("it can process multipart/signed mime messages and verify the signature with extra parts at the end", async () => {
        const { body, verificationStatus, signatures, attachments } =
            await processMIME({
    ...
    it("it can parse message with encrypted subject", async () => {
        const { verificationStatus, body, signatures, encryptedSubject } =
            await processMIME({
