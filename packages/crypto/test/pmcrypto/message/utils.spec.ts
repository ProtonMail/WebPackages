...
import { removeTrailingSpaces } from "../../../src/pmcrypto/message/utils.ts";
...
-----END PGP MESSAGE-----`);
        expect(x).to.deep.equal(
            new Uint8Array([
                140, 13, 4, 9, 3, 2, 166, 142, 200, 241, 106, 172, 121, 180,
                201, 210, 74, 1, 154, 110, 191, 162, 167, 71, 92, 149, 61, 104,
                247, 190, 62, 143, 167, 147, 249, 56, 252, 246, 107, 46, 105,
                87, 195, 61, 165, 134, 160, 96, 24, 9, 88, 246, 188, 85, 74,
                210, 193, 174, 13, 7, 159, 238, 96, 146, 28, 135, 35, 208, 124,
                230, 179, 53, 4, 70, 23, 67, 202, 79, 228, 36, 133, 185, 60, 27,
                39, 115, 182, 179, 174, 221, 205,
            ]),
        );
    });

    it("armorBytes - it can correctly enarmor a message", async () => {
        const armored = await armorBytes(
            new Uint8Array([
                140, 13, 4, 9, 3, 2, 166, 142, 200, 241, 106, 172, 121, 180,
                201, 210, 74, 1, 154, 110, 191, 162, 167, 71, 92, 149, 61, 104,
                247, 190, 62, 143, 167, 147, 249, 56, 252, 246, 107, 46, 105,
...
-----END PGP MESSAGE-----`;
        ...
        const pkesk = await readMessage({
            binaryMessage: packets.asymmetric[0],
        });
        expect(pkesk.packets[0]).to.be.instanceOf(
            PublicKeyEncryptedSessionKeyPacket,
        );
        const aeadData = await readMessage({
            binaryMessage: packets.encrypted[0],
        });
