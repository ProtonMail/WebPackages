...
describe("message verification", () => {
    it("verifyMessage - it verifies a message with multiple signatures", async () => {
        ...
        const {
            ...
            signature: await readSignature({
                armoredSignature: detachedSignatureFromTwoKeys,
            }),
    ...
    it("verifyMessage - it verifies a message with multiple signatures and returns the timestamp of the valid signature", async () => {
        ...
        const validSignature = signaturePackets.find((sigPacket) =>
            sigPacket.issuerKeyID.equals(publicKey1.getKeyID()),
        );
        const invalidSignature = signaturePackets.find((sigPacket) =>
            sigPacket.issuerKeyID.equals(publicKey2.getKeyID()),
        );
    ...
    it("verifyMessage - it does not verify a message given wrong public key", async () => {
        ...
        const { verificationStatus, signatureTimestamp, signatures, errors } =
            await verifyMessage({
                textData: "hello world",
                signature: await readSignature({
                    armoredSignature: detachedSignatureFromTwoKeys,
        ...
        errors?.forEach((err) =>
            expect(err.message).to.match(/Could not find signing key/),
        );
...
`;
        ...
        const { verificationStatus, signatureTimestamp, signatures, errors } =
            await verifyCleartextMessage({
                cleartextMessage: await readCleartextMessage({
                    cleartextMessage,
                }),
                verificationKeys: [publicKey],
            });
        ...
        expect(errors).to.be.undefined;
        expect(signatureTimestamp).to.deep.equal(
            new Date("Fri, 25 Mar 2022 11:12:34 GMT"),
        );
    });
