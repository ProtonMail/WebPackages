...
-----END PGP MESSAGE-----`;
        ...
        expect(decryptionResult.verificationStatus).to.equal(
        ...
        await expect(decryptWithWrongPassword).rejects.toThrow(
...
`;
        ...
        expect(decryptionResult.verificationErrors?.[0]).to.match(
...
-----END PGP MESSAGE-----`,
        ...
        expect(decryptionResult.data).to.deep.equal(
            utf8StringToUint8Array("hello world"),
        );
        expect(decryptionResult.signatures).to.have.length(0);
        expect(decryptionResult.verificationErrors).to.not.exist;
        expect(decryptionResult.verificationStatus).to.equal(
...
-----END PGP MESSAGE-----`;
    ...
    it("encryptMessage - should compress with zlib only if given `compress: true`", async () => {
        ...
        expect(decryptedMessage.packets.findPacket(enums.packet.compressedData))
            .to.be.undefined;

        // request compression
        const { message: compressedArmoredMessage } =
            await CryptoApiImplementation.encryptMessage({
                textData: "hello world",
                passwords: password,
                compress: true,
            });
        const compressedEncryptedMessage = await openpgp_readMessage({
            armoredMessage: compressedArmoredMessage,
        });
        const compressedDecryptedMessage =
            await compressedEncryptedMessage.decrypt([], [password]);
        ...
        // @ts-expect-error undeclared algorithm field
    ...
    it("encryptMessage - it does not encrypt with SEIPDv2 by default", async () => {
        ...
        const { message: armoredEncryptedWithSEIPDv1 } =
            await CryptoApiImplementation.encryptMessage({
                binaryData: utf8StringToUint8Array("Hello world!"),
                encryptionKeys: privateKey,
            });

        const encryptedWithSEIPDv1 = await openpgp_readMessage({
        ...
        // @ts-expect-error missing `version` field declaration
    ...
    it("encryptMessage/decryptMessage - should encrypt and decrypt text and binary data", async () => {
        ...
        const { message: encryptedArmoredMessage } =
            await CryptoApiImplementation.encryptMessage({
                textData: "hello world",
        ...
        const textDecryptionResult =
            await CryptoApiImplementation.decryptMessage({
        ...
        expect(textDecryptionResult.verificationStatus).to.equal(
            VERIFICATION_STATUS.NOT_SIGNED,
        );

        const { message: encryptedBinaryMessage } =
            await CryptoApiImplementation.encryptMessage({
        ...
        const binaryDecryptionResult =
            await CryptoApiImplementation.decryptMessage({
                binaryMessage: encryptedBinaryMessage,
                decryptionKeys: privateKeyRef,
                format: "binary",
            });
        expect(binaryDecryptionResult.data).to.deep.equal(
    ...
    it("signMessage/verifyMessage - output binary signature and data should be transferred", async () => {
        ...
        expect(verificationResult.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        );

        const invalidVerificationResult =
            await CryptoApiImplementation.verifyMessage({
                textData: "not signed data",
                binarySignature,
                verificationKeys: privateKeyRef,
                format: "binary",
            });
        expect(invalidVerificationResult.data).to.deep.equal(
            utf8StringToUint8Array("not signed data"),
        );
        expect(invalidVerificationResult.signatures).to.have.length(1);
        expect(invalidVerificationResult.errors).to.have.length(1);
        expect(invalidVerificationResult.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_INVALID,
        );
    });
    ...
    it("signMessage/verifyMessage - with context", async () => {
        ...
        const verificationValidContext =
            await CryptoApiImplementation.verifyMessage({
                textData,
                armoredSignature,
                verificationKeys: privateKeyRef,
        ...
        const verificationMissingContext =
            await CryptoApiImplementation.verifyMessage({
                textData,
                armoredSignature,
                verificationKeys: privateKeyRef,
        ...
        expect(verificationValidContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_VALID,
        );
        expect(verificationMissingContext.verificationStatus).to.equal(
            VERIFICATION_STATUS.SIGNED_AND_INVALID,
        );
        // check errors
        expect(verificationValidContext.errors).to.be.undefined;
        expect(verificationMissingContext.errors).to.have.length(1);
        expect(verificationMissingContext.errors?.[0]).to.match(
            /Unknown critical notation: context@proton/,
        );
    ...
    it("verifyMessage - it verifies a message ten seconds in the future", async () => {
        ...
        const { data: signed, verificationStatus } =
            await CryptoApiImplementation.verifyMessage({
                textData: data,
                armoredSignature,
                verificationKeys: privateKeyRef,
...
-----END PGP SIGNATURE-----`;
        ...
        const verificationResult =
            await CryptoApiImplementation.verifyCleartextMessage({
    ...
    it("should encrypt/sign and decrypt/verify binary data with detached signatures", async () => {
        ...
        const decryptionResultWithEncryptedSignature =
            await CryptoApiImplementation.decryptMessage({
                binaryMessage: encryptedBinaryMessage,
        ...
        expect(decryptionResultWithEncryptedSignature.data).to.deep.equal(
            plaintext,
        );
        expect(
            decryptionResultWithEncryptedSignature.signatures,
        ).to.have.length(1);
        expect(decryptionResultWithEncryptedSignature.verificationErrors).to.not
            .exist;
        expect(
            decryptionResultWithEncryptedSignature.verificationStatus,
        ).to.equal(VERIFICATION_STATUS.SIGNED_AND_VALID);
    });
    ...
    it("should encrypt/sign and decrypt/verify with context", async () => {
        ...
        const { message: encryptedBinaryMessage } =
            await CryptoApiImplementation.encryptMessage(encryptionOptions);
    ...
    it("should support encrypting/decrypting using argon2", async () => {
        ...
        const sessionKey = {
            algorithm: enums.read(
                enums.symmetric,
                enums.symmetric.aes128,
            ) as Exclude<enums.symmetricNames, "plaintext">,
        ...
        expect(encrypted[skeskStartIndex + 2]).to.equal(
            S2kTypeForConfig.argon2,
        );

        const decryptedSessionKey =
            await CryptoApiImplementation.decryptSessionKey({
    ...
    it("generateSessionKey - should return session key of expected size", async () => {
        const sessionKey128 =
            await CryptoApiImplementation.generateSessionKeyForAlgorithm(
                "aes128",
            );
    ...
    it("generateKey/generateSessionKey/encryptMessage - seipdv2 integration - should only use aead encryption when appropriate config options are passed (aeadProtect, ignoreSEIPDv2FeatureFlag)", async () => {
        ...
        const sessionKeyWithAEAD =
            await CryptoApiImplementation.generateSessionKey({
        ...
        const anotherSessionKeyWithoutAEAD =
            await CryptoApiImplementation.generateSessionKey({
        ...
        const { message: seipdv2ArmoredMessage } =
            await CryptoApiImplementation.encryptMessage({
                textData: "test",
    ...
    it("processMIME - it can process multipart/signed mime messages and verify the signature", async () => {
        const mimeKeyRef = await CryptoApiImplementation.importPublicKey({
            armoredKey: mimeKey,
        });
        const { body, verificationStatus, signatures } =
            await CryptoApiImplementation.processMIME({
    ...
    it("processMIME - it can parse message with text attachment", async () => {
        const { verificationStatus, body, signatures, attachments } =
            await CryptoApiImplementation.processMIME({
        ...
        expect(uint8ArrayToUtf8String(attachment.content)).to.equal(
            "this is the attachment text\n",
        );
    });

    it("processMIME - it can parse message with empty signature", async () => {
        const { body, signatures, verificationStatus, attachments } =
            await CryptoApiImplementation.processMIME({
...
-----END PGP MESSAGE-----`;
        ...
        const signedMessageInfo = await CryptoApiImplementation.getMessageInfo({
            armoredMessage: signedMessage,
        });
        expect(signedMessageInfo.encryptionKeyIDs).to.deep.equal([]);
        expect(signedMessageInfo.signingKeyIDs).to.deep.equal([
            "6998e6a67b21b0bf",
        ]);

        const encryptedMessageInfo =
            await CryptoApiImplementation.getMessageInfo({
                armoredMessage: encryptedMessage,
            });
        expect(encryptedMessageInfo.encryptionKeyIDs).to.deep.equal([
            "99d4b3ce6df9b8e3",
        ]);
...
-----END PGP SIGNATURE-----`;
        ...
        const signatureInfo = await CryptoApiImplementation.getSignatureInfo({
            armoredSignature,
        });
...
-----END PGP PRIVATE KEY BLOCK-----`;
        const encryptedPrivateKey = await openpgp_encryptKey({
            privateKey: await openpgp_readPrivateKey({
                armoredKey: armoredDecryptedPrivateKey,
            }),
            passphrase: "passphrase",
        });

        const publicKeyInfo = await CryptoApiImplementation.getKeyInfo({
            armoredKey: armoredPublicKey,
        });
        expect(publicKeyInfo.keyIsPrivate).to.be.false;
        expect(publicKeyInfo.keyIsDecrypted).to.be.null;
        const decryptedKeyInfo = await CryptoApiImplementation.getKeyInfo({
            armoredKey: armoredDecryptedPrivateKey,
        });
        expect(decryptedKeyInfo.keyIsPrivate).to.be.true;
        expect(decryptedKeyInfo.keyIsDecrypted).to.be.true;

        const encryptedKeyInfo = await CryptoApiImplementation.getKeyInfo({
            armoredKey: encryptedPrivateKey.armor(),
        });
        expect(encryptedKeyInfo.keyIsPrivate).to.be.true;
        expect(encryptedKeyInfo.keyIsDecrypted).to.be.false;
        expect(encryptedKeyInfo.fingerprint).to.equal(
            encryptedPrivateKey.getFingerprint(),
        );
    ...
    it("getArmoredSignature - it returns a valid armored signature", async () => {
        ...
        const armoredSignature =
            await CryptoApiImplementation.getArmoredSignature({
    ...
    it("isExpiredKey/canKeyEncrypt - it can correctly detect an expired key", async () => {
        ...
        expect(
            await CryptoApiImplementation.isExpiredKey({
                key: expiringKeyRef,
                date: now,
            }),
        ).to.be.false;
        ...
        expect(
            await CryptoApiImplementation.canKeyEncrypt({
                key: expiringKeyRef,
                date: now,
            }),
        ).to.be.true;
        ...
        expect(
            await CryptoApiImplementation.isExpiredKey({ key: keyReference }),
        ).to.be.false;
        expect(
            await CryptoApiImplementation.isExpiredKey({
                key: keyReference,
    ...
    it("isRevokedKey/canKeyEncrypt - it can correctly detect a revoked key", async () => {
        ...
        const keyRef = await CryptoApiImplementation.importPublicKey({
            armoredKey: key.armor(),
        });
        const revokedKeyRef = await CryptoApiImplementation.importPublicKey({
            armoredKey: armoredRevokedKey,
        });
        expect(
            await CryptoApiImplementation.isRevokedKey({
                key: revokedKeyRef,
                date: past,
            }),
        ).to.be.true;
    ...
    it("getAlgorithmInfo - it returns the expected values", async () => {
        ...
        expect(keyReference.getAlgorithmInfo()).to.deep.equal({
            algorithm: "ecdsa",
            curve: "nistP256",
        });
        expect(keyReference.subkeys[0].getAlgorithmInfo()).to.deep.equal({
            algorithm: "ecdh",
            curve: "nistP256",
        });
    ...
    it("computeHash", async () => {
        ...
        expect(testHashSHA256).to.equal(
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        const sourceKeyRef = await CryptoApiImplementation.importPublicKey({
        ...
        await CryptoApiImplementation.replaceUserIDs({
            sourceKey: sourceKeyRef,
            targetKey: targetKeyRef,
        });

        const exportedSourceKey = await openpgp_readKey({
            armoredKey: await CryptoApiImplementation.exportPublicKey({
                key: sourceKeyRef,
            }),
        });
        ...
        expect(sourceKey.getUserIDs()).to.deep.equal(
            exportedSourceKey.getUserIDs(),
        );
        ...
        expect(targetKey.getUserIDs()).to.not.deep.equal(
            exportedTargetKey.getUserIDs(),
        );
        expect(sourceKey.getUserIDs()).to.deep.equal(
            exportedTargetKey.getUserIDs(),
        );
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        const updatedKeyRef =
            await CryptoApiImplementation.cloneKeyAndChangeUserIDs({
                privateKey: sourceKeyRef,
                userIDs: [{ email: "new1@pm.me" }, { email: "new2@pm.me" }],
            });
        expect(updatedKeyRef.getUserIDs()).to.deep.equal([
            "<new1@pm.me>",
            "<new2@pm.me>",
        ]);
        expect(updatedKeyRef.getUserIDs()).to.not.deep.equal(
            sourceKeyRef.getUserIDs(),
        );
        expect(updatedKeyRef.getFingerprint()).to.deep.equal(
            sourceKeyRef.getFingerprint(),
        );
        expect(updatedKeyRef.getSHA256Fingerprints()).to.deep.equal(
            await getSHA256Fingerprints(sourceKey),
        );
        ...
        expect(sourcePrimaryUser.user.userID).to.deep.equal(
            (await exportedSourceKey.getPrimaryUser()).user.userID,
        );
        // target key users should have changed
        const updatedPrimaryUser = await exportedUpdatedKey.getPrimaryUser();
        expect(exportedUpdatedKey.getUserIDs()).to.deep.equal([
            "<new1@pm.me>",
            "<new2@pm.me>",
        ]);
        expect(sourceKey.getUserIDs()).to.not.deep.equal(
            exportedUpdatedKey.getUserIDs(),
        );
        expect(sourcePrimaryUser.user.userID).to.not.deep.equal(
            updatedPrimaryUser.user.userID,
        );
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        expect(exportedUpdatedKey.write()).to.deep.equal(
            exportedOriginalKey.write(),
        );
    });
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        const { proxyInstances, forwardeeKey } =
            await CryptoApiImplementation.generateE2EEForwardingMaterial({
                forwarderKey: bobKey,
                userIDsForForwardeeKey: {
                    email: "bob@test.com",
                    comment: "Forwarding from Bob",
                },
                passphrase: "passphrase",
            });
        ...
        expect(proxyInstances[0].forwarderKeyFingerprint.toHex()).to.include(
...
-----END PGP PRIVATE KEY BLOCK-----`,
    ...
    it("doesKeySupportE2EEForwarding - returns true on newly generated key", async () => {
        ...
        const bobKey = await CryptoApiImplementation.generateKey({
            userIDs: { email: "bob@test.com" },
        });

        expect(
            await CryptoApiImplementation.doesKeySupportE2EEForwarding({
                forwarderKey: bobKey,
            }),
        ).to.be.true;
    });
...
`,
        ...
        const charlieKeyEncrypted =
            await CryptoApiImplementation.importPublicKey({
                armoredKey: `-----BEGIN PGP PRIVATE KEY BLOCK-----
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        await expect(
            CryptoApiImplementation.isE2EEForwardingKey({ key: signOnlyKey }),
        ).resolves.toBe(false);
        await expect(
            CryptoApiImplementation.isE2EEForwardingKey({
                key: charlieKeyEncrypted,
            }),
        ).resolves.toBe(true);
        await expect(
            CryptoApiImplementation.isE2EEForwardingKey({ key: charlieKey }),
        ).resolves.toBe(true);
        await expect(
            CryptoApiImplementation.isE2EEForwardingKey({ key: bobKey }),
        ).resolves.toBe(false);
    });

    describe("Key management API", () => {
        it("version type inference of generated key", async () => {
            const privateKeyRefV4: PrivateKeyReferenceV4 =
                await CryptoApiImplementation.generateKey({
                    userIDs: { name: "name", email: "email@test.com" },
                });
            expect(privateKeyRefV4.isPrivateKeyV4()).to.be.true;
            const privateKeyRefV6: PrivateKeyReferenceV6 =
                await CryptoApiImplementation.generateKey({
                    userIDs: { name: "name", email: "email@test.com" },
                    config: { v6Keys: true },
                });
        ...
        it("can export a generated key", async () => {
            ...
            expect(decryptedKeyFromArmored.isDecrypted()).to.be.true;
            // @ts-expect-error missing `s2k` field definition
        ...
        it("can export an imported key", async () => {
            ...
            const importedKeyRef =
                await CryptoApiImplementation.importPrivateKey({
                    armoredKey: keyToImport.armor(),
                    passphrase,
                });
            expect(importedKeyRef.getCreationTime()).to.deep.equal(
                keyToImport.getCreationTime(),
            );
            expect(
                importedKeyRef.subkeys.map((subkey) =>
                    subkey.getAlgorithmInfo(),
                ),
            ).to.deep.equal(
                keyToImport.subkeys.map((subkey) => subkey.getAlgorithmInfo()),
            );
            expect(importedKeyRef.getUserIDs()).to.deep.equal([
                "name <email@test.com>",
            ]);
            const armoredPublicKey =
                await CryptoApiImplementation.exportPublicKey({
                    key: importedKeyRef,
                });
            const exportedPublicKey = await openpgp_readKey({
                armoredKey: armoredPublicKey,
            });
            expect(exportedPublicKey.isPrivate()).to.be.false;
            expect(exportedPublicKey.getKeyID().toHex()).equals(
                importedKeyRef.getKeyID(),
            );
            expect(exportedPublicKey.getKeyID().equals(keyToImport.getKeyID()));

            const exportPassphrase = "another passphrase";
            const armoredPrivateKey =
                await CryptoApiImplementation.exportPrivateKey({
                    privateKey: importedKeyRef,
                    passphrase: exportPassphrase,
                });
            const exportedPrivateKey = await openpgp_readPrivateKey({
                armoredKey: armoredPrivateKey,
            });
            expect(
                exportedPrivateKey.getKeyID().equals(keyToImport.getKeyID()),
            );
        ...
        it("exports an unencrypted key only when given a null passphrase", async () => {
            ...
            const armoredEncryptedKey =
                await CryptoApiImplementation.exportPrivateKey({
                    privateKey: keyReference,
                    passphrase: "passphrase",
                });
            const encryptedKey = await openpgp_readPrivateKey({
                armoredKey: armoredEncryptedKey,
            });
            expect(encryptedKey.isDecrypted()).to.be.false;
            const armoredUnencryptedKey =
                await CryptoApiImplementation.exportPrivateKey({
                    privateKey: keyReference,
                    passphrase: null,
                });
            const unencryptedKey = await openpgp_readPrivateKey({
                armoredKey: armoredUnencryptedKey,
            });
        ...
        it("cannot import or export a public key as a private key", async () => {
            ...
            expect(
                exportedPublicKey
                    .getKeyID()
                    .equals(publicKeyToImport.getKeyID()),
            );
        });
...
-----END PGP PRIVATE KEY BLOCK-----`,
            ...
            expect(
                reformattedKeyRef
                    .getUserIDs()
                    .includes("<reformatted@worker.com>"),
            );
            expect(originalKeyRef.getUserIDs()).to.have.length(1);
            expect(originalKeyRef.getUserIDs()).includes("<test@worker.com>");
            ...
            const reformattedKeyRef = await CryptoApiImplementation.reformatKey(
                {
                    privateKey,
                    userIDs: [
                        { name: "reformatted", email: "reformatteed@test.com" },
                    ],
                },
            );
            const armoredReformattedKey =
                await CryptoApiImplementation.exportPublicKey({
                    key: reformattedKeyRef,
                });
            const reformattedKey = await openpgp_readKey({
                armoredKey: armoredReformattedKey,
            });
            const primaryUser = await reformattedKey.getPrimaryUser();
            expect(primaryUser.user.userID?.userID).to.equal(
                "reformatted <reformatteed@test.com>",
            );
            expect(
                (await reformattedKey.getPrimaryUser()).selfCertification
                    .created,
            ).to.deep.equal(date);
        });

        it("isWeak() - it correctly marks a weak key", async () => {
            const weakKeyReference =
                await CryptoApiImplementation.importPublicKey({
                    armoredKey: rsa512BitsKey,
                });
            expect(weakKeyReference.isWeak()).to.be.true;

            const keyReference = await CryptoApiImplementation.importPublicKey({
                armoredKey: ecc25519Key,
            });
        ...
        it("getSHA256Fingerprints - it returns the expected fingerprints", async () => {
            ...
            expect(sha256Fingerprings).to.deep.equal(
                await getSHA256Fingerprints(key),
            );
        });
        ...
        it("equals - can ignore third-party certifications", async () => {
            const publicKey = await openpgp_readKey({
            ...
            const certifiedPublicKeyRef =
                await CryptoApiImplementation.importPublicKey({
                    armoredKey: keyWithThirdPartyCertifications,
                });

            expect(certifiedPublicKeyRef.equals(publicKeyRef, false)).to.be
                .false;
        ...
        it("clearKey - cannot reference a cleared key", async () => {
            ...
            await expect(
                CryptoApiImplementation.exportPublicKey({ key: privateKeyRef }),
        ...
        it("clearKeyStore - cannot reference any key after clearing the store", async () => {
            ...
            await expect(
                CryptoApiImplementation.exportPublicKey({
                    key: privateKeyRef1,
                }),
            ).rejects.toThrow(/Key not found/);
            await expect(
