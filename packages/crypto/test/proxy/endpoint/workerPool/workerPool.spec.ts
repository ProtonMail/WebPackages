...
describe("Worker Pool", () => {
    ...
    it("should encrypt/sign and decrypt/verify text and binary data", async () => {
        ...
        const bobKeyRef = await CryptoWorkerPool.generateKey({
            userIDs: { name: "bob", email: "bob@test.com" },
        });

        const { message: encryptedArmoredMessage } =
            await CryptoWorkerPool.encryptMessage({
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        const sourceKeyRef = await CryptoWorkerPool.importPublicKey({
        ...
        await CryptoWorkerPool.replaceUserIDs({
        ...
        const exportedTargetKeys = await Promise.all(
            new Array(poolSize).fill(null).map(async () =>
                openpgp_readKey({
                    armoredKey: await CryptoWorkerPool.exportPublicKey({
                        key: targetKeyRef,
                    }),
                }),
            ),
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        exportedTargetKeys.forEach((exportedTargetKey) => {
            expect(exportedTargetKey.getUserIDs()).to.deep.equal([
                "<updated@worker.com>",
            ]);
        });
    });
    ...
    describe("Key management API", () => {
        ...
        it("can export an imported key", async () => {
            ...
            const armoredPublicKey = await CryptoWorkerPool.exportPublicKey({
                key: importedKeyRef,
            });
...
-----END PGP PRIVATE KEY BLOCK-----`,
        ...
        it("clearKey - cannot reference a cleared key", async () => {
            ...
            expect(
                await CryptoWorkerPool.exportPublicKey({ key: privateKeyRef }),
            ).length.above(0);
            await CryptoWorkerPool.clearKey({ key: privateKeyRef });

            await expect(
                CryptoWorkerPool.exportPublicKey({ key: privateKeyRef }),
        ...
        it("clearKeyStore - cannot reference any key after clearing the store", async () => {
            ...
            expect(
                await CryptoWorkerPool.exportPublicKey({ key: privateKeyRef1 }),
            ).length.above(0);
            expect(
                await CryptoWorkerPool.exportPublicKey({ key: privateKeyRef2 }),
            ).length.above(0);
            await CryptoWorkerPool.clearKeyStore();

            await expect(
                CryptoWorkerPool.exportPublicKey({ key: privateKeyRef1 }),
            ).rejects.toThrow(/Key not found/);
            await expect(
                CryptoWorkerPool.exportPublicKey({ key: privateKeyRef2 }),
            ).rejects.toThrow(/Key not found/);
        });
    });
});
