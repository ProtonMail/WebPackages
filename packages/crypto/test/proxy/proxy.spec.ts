...
import type { CryptoApiInterface } from "../../src/index.ts";
import {
    CryptoProxy,
    VERIFICATION_STATUS,
    updateServerTime,
...
describe("CryptoProxy", () => {
    ...
    it("setEndpoint - should throw if already set", async () => {
        ...
        expect(() => CryptoProxy.setEndpoint(api)).to.throw(
            /already initialised/,
        );
...
`;
        ...
        const verificationKeys = await CryptoProxy.importPublicKey({
            armoredKey,
        });
        const { verificationStatus, data: verifiedData } =
        ...
        expect(verifiedData).to.equal(
            "BEGIN:VCARD\nVERSION:4.0\nFN;PREF=1:\nEND:VCARD",
        );

        // test fallback verification
        const {
            verificationStatus: verifiedFallback,
            data: verifiedDataFallback,
        } = await CryptoProxy.verifyMessage({
            textData,
