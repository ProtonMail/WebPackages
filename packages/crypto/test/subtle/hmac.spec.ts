...
import { importKey, signData, verifyData } from "../../src/subtle/hmac.ts";
...
describe("Subtle - HMAC-SHA256 helpers", () => {
    it("importKey - throws on short keys", async () => {
        ...
        await expect(importKey(secretBytesTooShort)).rejects.toThrow(
            /key is too short/,
        );
    });
    ...
    it("importKey/signData/verifyData - correctly confirms authenticity of signed data", async () => {
        ...
        await expect(
            verifyData(key, signed, new Uint8Array([1, 2, 3])),
        ).resolves.toBe(false);
    });
});
