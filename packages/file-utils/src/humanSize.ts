...
export const getLongSizeFormat = (key: SizeUnits, n: number) => {
    ...
    if (key === "KB") {
        return c("file size format, long").ngettext(
            msgid`Kilobyte`,
            `Kilobytes`,
            n,
        );
...
/**
 * Formats a byte count as a human-readable size string (e.g. `12.34 MB`).
 * The unit is auto-detected from `bytes` unless an explicit `unit` is given,
 * capped at GB by default (override via `unitOptions.max`).
 */
export const humanSize = ({
    ...
    const fractionDigits =
        maybeFraction === undefined && unit === "B" ? 0 : maybeFraction;
